...
def split_train_val(datafile):
    ...
    train_list = [
        (row.Image, row.Name, row.Contour) for row in df_train.itertuples()
    ]
    df_valid = df[~train_idx]
    valid_list = [
        (row.Image, row.Name, row.Contour) for row in df_valid.itertuples()
    ]
...
class CoarseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        probs = utils.points_to_mask(
            pts_xy_int, radii_int, occluded, image.shape[0:2]
        )
        ...
        width = int(np.round(np.linalg.norm(
            np.array([x0, y0]) - np.array([x1, y0])
        )))
        height = int(np.round(np.linalg.norm(
            np.array([x0, y0]) - np.array([x0, y1])
        )))
        ...
        new_pts_xy_int = np.round(np.array([self.width/width, self.height/height]) * new_pts_xy_int).astype(np.int32)
        target = utils.points_to_mask(
            new_pts_xy_int, radii_int, occluded, np.array([self.height, self.width])
        )

        crop = utils.sub_image(image, center, theta, width, height,
        ...
        _, target = cv2.threshold(
            target, 0, 1, cv2.THRESH_BINARY | cv2.THRESH_OTSU
        )
...
class CoarseEvalDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        crop, _ = utils.crop_with_padding(
            img, row.x, row.y, row.w, row.h, self.pad
        )
        if row.Mirror:
            crop = crop[:, ::-1]
        crop = cv2.resize(crop, (self.width, self.height),
...
class FineDataset(data.Dataset):
    def __init__(self, input_list, cachedir,
                 height1, width1, height2, width2,
                 patch_size, num_samples, num_fixed):
    ...
    def __getitem__(self, index):
        ...
        target = join(self.cachedir, '%s.h5' % (
            splitext(basename(contour_fpath))[0]))
        if isfile(target):
            ...
            probs = utils.points_to_mask(
                pts_xy_int, radii_int, occluded, img.shape[0:2]
            )
            ...
            resized_probs = cv2.resize(part_probs, (self.width1, self.height1),
                                       interpolation=cv2.INTER_AREA)
            peaks_ij, normals, is_max = algo.control_points(resized_probs)
            pts_xy = peaks_ij[is_max][:, ::-1]
            pts_normal_xy = normals[is_max][:, ::-1]

            M = np.array([[1. * part_img.shape[1] / self.width1, 0.],
                          [0., 1. * part_img.shape[0] / self.height1]])
            ...
            height_ratio = 1. * part_img.shape[0] / self.height2
            width_ratio = 1. * part_img.shape[1] / self.width2
        ...
        def stitch(width, height):
            ...
            fx = width * bernstein_poly(x, coeffs, np.array([0., height]))
        ...
        targets = np.empty((pts_xy.shape[0], patch_size, patch_size),
                           dtype=np.float32)
        for i, _ in enumerate(pts_xy):
            ...
            pos_fill = crop[:, -x0:]
            neg_fill = crop[:, :x0]
            stitched = matte * pos_source + (1. - matte) * neg_source
            ...
            idx = np.argwhere((0. < matte[:, :, 0]) & (matte[:, :, 0] < 1.))
            transition[idx[:, 0], x0 + idx[:, 1]] = 1.

            # The center of the synthetic patch is the center of the original
            # crop without the real boundary and one overlap region.
            synth_center = np.array([
                crop_width - 4. * self.num_fixed, crop_height
            ]) / 2.
            theta = -thetas[i]
            # The axis-aligned patch cropped from inside the synthetic patch.
            patch = utils.sub_image(
                synth, synth_center, theta, patch_size, patch_size
            )
            ...
            real_center = np.array([crop_width, crop_height]) / 2.
            real = utils.sub_image(
                crop, real_center, theta, patch_size, patch_size
            )
            synth_patches[i] = patch.transpose(2, 0, 1) / 255.
            real_patches[i] = real.transpose(2, 0, 1) / 255.
            targets[i] = target
            ...
            if visualize:
                ...
                trans = cv2.getRotationMatrix2D(
                    tuple(synth_center), np.rad2deg(theta), 1.
                )
                poly_rot = cv2.transform(np.array([poly]), trans)[0]
                poly_rot -= (synth_center - patch_size / 2.)

                valid_idx, = np.where((poly_rot[:, 0] >= 0) &
                ...
                xy = pts_xy[i] - np.array([
                    img_crop_width /  2., img_crop_height / 2.])
                axarr[:, 0] = plt.subplot2grid((2, 2), (0, 0), rowspan=3)
                axarr[0, 0].imshow(img[:, :, ::-1])
                axarr[0, 0].scatter(pts_xy[i, 0], pts_xy[i, 1],
                                    color='red', s=5)
                crop_rect = mpl_patches.Rectangle(
                    xy, img_crop_width, img_crop_height,
                ...
                xy = pts_xy[i] - np.array([
                    width_ratio * patch_size /  2.,
                    height_ratio * patch_size / 2.])
                axarr[0, 0].add_patch(crop_rect)
                patch_rect = mpl_patches.Rectangle(
                    xy, width_ratio * patch_size, height_ratio * patch_size,
                    linewidth=1, edgecolor='red', facecolor='none')
                axarr[0, 0].add_patch(patch_rect)

                xy = synth_center - np.array([
                    patch_size / 2., patch_size / 2.])
                crop_rect = mpl_patches.Rectangle(
                    xy, patch_size, patch_size,
                    linewidth=1, edgecolor='red', facecolor='none')
                crop_trans = Affine2D().rotate_around(
                ...
                axarr[1, 1].plot(crop_width - (x1 + x0) + poly[:, 0],
                                 poly[:, 1], color='red')
...
class RegressionDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Approximately the center of the contour bounding box.
        center = (x0 + (x1 - x0) / 2., y0 + (y1 - y0) / 2.)
        crop = utils.sub_image(image, center, 0., width, height,
                               border_mode=cv2.BORDER_CONSTANT)
        ...
        start[0] /= (1. * self.width)
        start[1] /= (1. * self.height)
        end[0] /= (1. * self.width)
        end[1] /= (1. * self.height)
